...
function KpiCard({ label, value, suffix, sub, delta, sign = 'pos', spark, badge }) {
  const deltaCls = sign === 'pos' ? 'text-[var(--accent)]' : 'text-[var(--bad)]';
  return (
    <div className="border hairline rounded-xl bg-[var(--bg-1)] px-4 py-3 grid grid-cols-[1fr_auto_auto] items-center gap-5 hover:border-[var(--line-2)] transition">
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          ...
          {badge && <div className="badge badge-buy">{badge}</div>}
        </div>
      ...
      <div className="flex items-baseline gap-1 justify-end">
        <span className="display text-[40px] tnum">{value}</span>
        {suffix && <span className="mono text-[12px] text-[var(--ink-mute)]">{suffix}</span>}
      </div>
      {spark ? <MiniSpark values={spark} /> : <span className="w-[64px]"></span>}
    </div>
  );
}

function KPIRow({ kpis, finalNet, equityData, benchLabel = 'Benchmark' }) {
  ...
  const cagrVsBench = (kpis.cagr.gem - kpis.cagr.bench).toFixed(1);
    ...
    <div className="grid grid-cols-12 gap-3">
      ...
      <div className="col-span-12 lg:col-span-6 border hairline rounded-xl bg-[var(--bg-1)] relative overflow-hidden">
        ...
        <div className="relative p-5 flex flex-col gap-4 h-full">
          <div className="flex items-center justify-between">
            ...
            <div className="badge badge-buy">15-letnia</div>
          </div>

          <div className="flex items-end gap-6 flex-wrap">
            <div className="flex items-baseline gap-1">
              <span className="display text-[88px] md:text-[104px] tnum text-[var(--ink)]">
                {kpis.cagr.gem.toFixed(1)}
              </span>
              <span className="display text-[44px] text-[var(--accent)] tnum">%</span>
            </div>

            <div className="flex flex-col gap-2 pb-3">
              <div className="flex items-center gap-2">
                <span className="text-[var(--accent)] mono text-[12px] tnum arrow-up">+{cagrVsBench} pp vs benchmark</span>
              </div>
              <div className="grid grid-cols-2 gap-x-6 gap-y-1 mono text-[11px]">
                ...
                <span className="text-[var(--ink-mute)]">Net portfolio</span>
                <span className="tnum text-[var(--ink-dim)]">{(finalNet/1000).toFixed(0)}k PLN</span>
                <span className="text-[var(--ink-mute)]">CAGR po IKE</span>
                <span className="tnum text-[var(--accent)]">{(kpis.cagr.gem * 1.025).toFixed(1)}%</span>
              </div>
            </div>
          </div>
      ...
      <div className="col-span-12 lg:col-span-6 grid grid-cols-1 gap-3">
        <KpiCard
          ...
          suffix="%"
          sign="pos"
          delta={`+${(kpis.maxDd.gem - kpis.maxDd.bench).toFixed(1)} pp lepiej`}
        ...
        <KpiCard
          ...
          sign="pos"
          delta={`+${(kpis.sharpe.gem - kpis.sharpe.bench).toFixed(2)}`}
        ...
        <KpiCard
          ...
          sign="pos"
          delta={`−${(kpis.ulcer.bench - kpis.ulcer.gem).toFixed(1)} vs bench`}
